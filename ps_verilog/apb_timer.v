module apb_timer (
    ...
    always @(posedge PCLK or negedge PRESETn) begin
        if (~PRESETn)
            reg_curr_val <= 32'h0; // Corrected from {32{2'h20}}
    ...
    always @(posedge PCLKG or negedge PRESETn) begin
        if (~PRESETn)
            reg_reload_val <= 32'h0;
        else if (write_enable08)
            reg_reload_val <= PWDATA[31:0];
    end

    // Read operation, partitioned into two parts
    assign pid3_value = ARM_CMSDK_APB_TIMER_PID3;
    always @(PADDR or reg_ctrl or reg_reload_val or reg_timer_int or ECOREVNUM or pid3_value) begin
        if (PADDR[11:4] == 8'h00) begin
            case (PADDR[3:2])
                2'h0: read_mux_byte0 = {4'h0, reg_ctrl};
                2'h1: read_mux_byte0 = 8'h0;
                2'h2: read_mux_byte0 = reg_reload_val[7:0];
                2'h3: read_mux_byte0 = {7'h0, reg_timer_int};
                default: read_mux_byte0 = 8'h0;
            endcase
        end
        else if (PADDR[11:6] == 6'h3F) begin
            case (PADDR[5:2])
                4'h0, 4'h1, 4'h2, 4'h3: read_mux_byte0 = 8'h0;
    ...
    always @(PADDR or read_mux_byte0_reg or reg_curr_val or reg_reload_val) begin
        if (PADDR[11:4] == 8'h00) begin
            case (PADDR[3:2])
                2'b01: read_mux_word = reg_curr_val;
                2'b10: read_mux_word = {reg_reload_val[31:8], read_mux_byte0_reg};
                2'b00, 2'b11: read_mux_word = {24'h0, read_mux_byte0_reg};
                default: read_mux_word = 32'h0;
            endcase
        end
        else begin
            read_mux_word = {24'h0, read_mux_byte0_reg};
        end
    end

    // Output read data to APB
    assign PRDATA = (read_enable) ? read_mux_word : 32'h0;
    ...
    // Decrement counter
    always @(write_enable04 or PWDATA or dec_ctrl or reg_curr_val or reg_reload_val) begin
        if (write_enable04)
            nxt_curr_val = PWDATA[31:0]; // Software write to timer
        else if (dec_ctrl) begin
            if (reg_curr_val == 32'h0)
                nxt_curr_val = reg_reload_val; // Reload
            else
                nxt_curr_val = reg_curr_val - 1; // Decrement
        end
        else
